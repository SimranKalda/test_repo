...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  5 12:59:18 2024
...
def perform_data_quality_checks(df):
    ...
    if duplicate_count > 0:
        logger.warning(f"Data contains {duplicate_count} duplicate records. Removing duplicates.")
    ...
    if missing_count.sum() > 0:
        ...
        # Strategy: Fill missing values or drop rows/columns
    ...
    if invalid_columns:
        ...
        # Example: Convert all numeric-looking columns to floats
    ...
    # Fix numeric columns to two decimal places            
    ...
    numeric_columns = df.select_dtypes(include=["float64", "int64"]).columns
    for column in numeric_columns:
        if df[column].isnull().all():  # Skip if the column is entirely null
            continue
